...
@router.get("/{producto_lista_id}", response_model=ProductoListaResponse)
def get_producto_by_id(
    producto_lista_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    return ProductoListaService.get_producto_by_id(db, producto_lista_id, current_user)


@router.put("/{producto_lista_id}", response_model=ProductoListaResponse)
def update_producto(
    producto_lista_id: int,
...
@router.delete("/{producto_lista_id}")
def delete_producto(
    producto_lista_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    ProductoListaService.delete_producto(db, producto_lista_id, current_user)
