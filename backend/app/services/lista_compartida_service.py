...
class ListaCompartidaService:
    ...
    def compartir_lista(
        ...
        if HistorialListasRepository.get_by_lista_id(db, lista_id):
            raise HTTPException(
                ...
                detail="No puedes compartir una lista finalizada",
            )
        ...
        ya_compartida = ListaCompartidaRepository.get_by_lista_and_usuario(
            db,
            lista_id,
            usuario.id_usuario,
        )
        ...
        nueva_comparticion = ListaCompartida(
            ...
            tipo_compartido=compartir_data.tipo_compartido.value,
        )
    ...
    def salir_de_lista_compartida(
        ...
        if not comparticion:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="No tienes esta lista en tus compartidos",
            )
